...
static void low_level_init(struct netif *netif)
{
  ...
  heth.Instance = ETH;
  heth.Init.MACAddr = &lwipdev.mac[0];
...
/* 必须开启自动协商功能 */
	YT8512_StartAutoNego(&YT8512);

	HAL_Delay(2000);        /* 必须等待初始化 */
	PHYLinkState = YT8512_GetLinkState(&YT8512);

	if (PHYLinkState == YT8512_STATUS_READ_ERROR)
	{
			UserLog("ETH0 LINK ERROR");
	...
	else
	{
			switch (PHYLinkState)
			{
					case YT8512_STATUS_AUTO_100MBITS_FULLDUPLEX:
							UserLog("ETH0 AUTO 100M FULL");
							duplex = ETH_FULLDUPLEX_MODE;
							speed = ETH_SPEED_100M;
							break;

					case YT8512_STATUS_AUTO_100MBITS_HALFDUPLEX:
							UserLog("ETH0 AUTO 100M HALF");
							duplex = ETH_HALFDUPLEX_MODE;
							speed = ETH_SPEED_100M;
							break;

					case YT8512_STATUS_AUTO_10MBITS_FULLDUPLEX:
							UserLog("ETH0 AUTO 10M FULL");
							duplex = ETH_FULLDUPLEX_MODE;
							speed = ETH_SPEED_10M;
							break;

					case YT8512_STATUS_AUTO_10MBITS_HALFDUPLEX:
							UserLog("ETH0 AUTO 10M HALF");
							duplex = ETH_HALFDUPLEX_MODE;
							speed = ETH_SPEED_10M;
							break;
					default:
  ...
  else if(!netif_is_link_up(netif) && (PHYLinkState > YT8512_STATUS_LINK_DOWN))
  {
    switch (PHYLinkState)
    {
			...
			case YT8512_STATUS_10MBITS_HALFDUPLEX:
				UserLog("ETH0 FORCE 10M HALF");
				duplex = ETH_HALFDUPLEX_MODE;
				speed = ETH_SPEED_10M;
				linkchanged = 1;
				break;
			case YT8512_STATUS_AUTONEGO_NOTDONE:
				UserLog("ETH0 AUTO NOTDONE");
				break;
			default:
				break;
    }
