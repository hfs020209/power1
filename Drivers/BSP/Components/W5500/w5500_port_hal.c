...
static uint8_t  txsize[8] = {8,2,1,1,1,1,1,1}; // tx buffer set K bytes
static uint8_t  rxsize[8] = {8,2,1,1,1,1,1,1}; // rx buffet set K bytes   
/**
...
void w5500_network_info_show(void)
{
    ...
    UserLog("w5500 network infomation:");
    UserLog("  -mac:%d:%d:%d:%d:%d:%d", info.mac[0], info.mac[1], info.mac[2], 
            info.mac[3], info.mac[4], info.mac[5]);
    UserLog("  -ip:%d.%d.%d.%d", info.ip[0], info.ip[1], info.ip[2], info.ip[3]);
    UserLog("  -sn:%d.%d.%d.%d", info.sn[0], info.sn[1], info.sn[2], info.sn[3]);
    UserLog("  -gw:%d.%d.%d.%d", info.gw[0], info.gw[1], info.gw[2], info.gw[3]);
    UserLog("  -dns:%d.%d.%d.%d", info.dns[0], info.dns[1], info.dns[2], info.dns[3]);
    
    if (info.dhcp == NETINFO_DHCP) {
        UserLog("  -dhcp_mode: dhcp");
    } else {
        UserLog("  -dhcp_mode: static");
    }
}
...
//		wiz_PhyConf phyconf1;
//		phyconf1.by = PHY_CONFBY_HW;
//		phyconf1.mode =    PHY_MODE_AUTONEGO;
//		ctlwizchip(CW_SET_PHYCONF, (wiz_PhyConf*)&phyconf1);
//		HAL_Delay(1000);
...
//		uint16_t pa = IK_SOCK_ALL;
//		ctlwizchip(CW_SET_INTRMASK, &pa);
    return 0;
}
